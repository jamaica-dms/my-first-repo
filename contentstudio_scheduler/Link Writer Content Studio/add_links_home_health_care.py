#!/usr/bin/env python3
"""
Home Health Care Blog — Live Link Writer
...
Two post slots: 1 AM PDT (08:00 UTC) and 2 PM PDT (21:00 UTC) — same row.
"""
...
def find_row_number(worksheet, date_str):
    """Find the 1-based row number for the Home Health Care row on date_str."""
    ...
    for i in range(date_row_idx, min(date_row_idx + 15, len(rows))):
        if len(rows[i]) > 2 and "home health care" in rows[i][2].lower():
            return i + 1
...
def main():
    ...
    print("=" * 58)
    print(f"  Home Health Care Blog — Live Link Writer")
    ...
    if not row_num:
        print(f"[STOP] Could not find Home Health Care row for {date_str} in sheet.")
        sys.exit(1)
