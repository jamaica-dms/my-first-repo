#!/usr/bin/env python3
"""
Home Care Blog — Live Link Writer
Runs every Monday at 12:15 PM PDT via GitHub Actions.
Pulls live post links from ContentStudio (Senior Care Marketing Max workspace)
and writes them to the PB Blogs 2026 Google Sheet.
Two post slots: 9 AM PDT (16:00 UTC) and 12 PM PDT (19:00 UTC) — same row.
"""
...
def get_home_care_posts(date_str, all_posts):
    """Return Home Care posts at 9 AM PDT (16:00 UTC) or 12 PM PDT (19:00 UTC) on date_str."""
    return [
        p for p in all_posts
        if (p.get("scheduling", {}).get("execute_time", "").startswith(f"{date_str}T16:00")
            or p.get("scheduling", {}).get("execute_time", "").startswith(f"{date_str}T19:00"))
        and "home care" in p.get("common", {}).get("content", {}).get("text", "").lower()
    ]
...
# ── Playwright — get SCMM LinkedIn link ───────────────────────────────────────
def get_scmm_linkedin_via_playwright(post_id, welton_li_url, post_text=""):
    try:
        from playwright.sync_api import sync_playwright
    except ImportError:
        print("  [WARN] Playwright not installed — SCMM LI link skipped.")
        return None

    post_url = f"https://app.contentstudio.io/senior-care-marketing-max/publisher/planner/list-view?plan_ids={post_id}"
...
def find_row_number(worksheet, date_str):
    """Find the 1-based row number for the Home Care row on date_str."""
    ...
    for i in range(date_row_idx, min(date_row_idx + 15, len(rows))):
        if len(rows[i]) > 2 and "home care" in rows[i][2].lower():
            return i + 1
...
def write_links_to_sheet(spreadsheet, row_num, links):
    columns = {
        "F": [("Welton FB",  links.get("Welton FB")),  ("SCMM FB",  links.get("SCMM FB"))],
        "G": [("Welton LI",  links.get("Welton LI")),  ("SCMM LI",  links.get("SCMM LI"))],
        "H": [("Welton X",   links.get("Welton X")),   ("SCMM X",   links.get("SCMM X"))],
        "I": [("Welton IG",  links.get("Welton IG")),  ("SCMM IG",  links.get("SCMM IG"))],
    }
...
def main():
    ...
    print("=" * 58)
    print(f"  Home Care Blog — Live Link Writer")
    print(f"  Date: {date_str}")
    print("=" * 58)

    print("\nFetching posts from ContentStudio...")
    all_posts = fetch_all_posts()
    hc_posts = get_home_care_posts(date_str, all_posts)
    print(f"Found {len(hc_posts)} Home Care post(s) at 9 AM / 12 PM PDT")

    if not hc_posts:
        print("[STOP] No Home Care posts found for this date.")
        sys.exit(0)

    links = extract_links(hc_posts)
    print("\nLinks from API:")
    for label, url in links.items():
        print(f"  {label}: {url}")

    if "SCMM LI" not in links:
        print("\nSCMM LI not in API — fetching via Playwright...")
        for post in hc_posts:
            ...
            if "linkedin" in platforms and "facebook" in platforms:
                ...
                candidate = get_scmm_linkedin_via_playwright(post_id, welton_li, post_text)
                if candidate:
                    links["SCMM LI"] = candidate
                print(f"  SCMM LI: {links.get('SCMM LI', 'NOT FOUND')}")
                break
        time.sleep(5)
    ...
    if not row_num:
        print(f"[STOP] Could not find Home Care row for {date_str} in sheet.")
        sys.exit(1)
