#!/usr/bin/env python3
"""
Windows & Doors Blog — Live Link Writer
...
def get_windows_doors_posts(date_str, all_posts):
    """Return Windows & Doors posts at 1 PM PDT (20:00 UTC) or 2 PM PDT (21:00 UTC) on date_str."""
    return [
        ...
        and "door" in p.get("common", {}).get("content", {}).get("text", "").lower()
    ]
...
def find_row_number(worksheet, date_str):
    """Find the 1-based row number for the Windows & Doors row on date_str."""
    ...
    for i in range(date_row_idx, min(date_row_idx + 15, len(rows))):
        if len(rows[i]) > 2 and "door" in rows[i][2].lower():
            return i + 1
...
def main():
    ...
    print("=" * 58)
    print(f"  Windows & Doors Blog — Live Link Writer")
    print(f"  Date: {date_str}")
    print("=" * 58)

    print("\nFetching posts from ContentStudio...")
    all_posts = fetch_all_posts()
    wd_posts = get_windows_doors_posts(date_str, all_posts)
    print(f"Found {len(wd_posts)} Windows & Doors post(s) at 1-2 PM PDT")

    if not wd_posts:
        print("[STOP] No Windows & Doors posts found for this date.")
        sys.exit(0)

    links = extract_links(wd_posts)
    print("\nLinks from API:")
    ...
    if not row_num:
        print(f"[STOP] Could not find Windows & Doors row for {date_str} in sheet.")
        sys.exit(1)
