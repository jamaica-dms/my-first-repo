#!/usr/bin/env python3
"""
Blog QA Checker — Senior Care Marketing Max Workspace (Home Care)
Runs 3-phase QA on Home Care blog posts scheduled for a given Monday.

Usage:
    python qa_workflow_blog_homecare_scmm.py 2026-05-18
"""
...
BLOG_DELAY = timedelta(days=21)  # article published 3 weeks before posting date

# 9 AM PST = 16:00 UTC, 12 PM PST = 19:00 UTC
TIME_9AM  = "T16:00:00"
TIME_12PM = "T19:00:00"

HOMECARE_HASHTAGS = "#seniorcare #seniorcaremarketingmax #ringringmarketing"

# (platform, name_fragment, expected_pst_time)
EXPECTED_CHANNELS = {
    "SCMM Facebook":        ("facebook",  "Senior Care Marketing Max", "09:00 AM"),
    "Welton SCMM Facebook": ("facebook",  "Welton Hong",               "09:00 AM"),
    "SCMM LinkedIn":        ("linkedin",  "Senior Care Marketing Max", "09:00 AM"),
    "SCMM Instagram":       ("instagram", "SeniorCare MarketingMax",   "09:00 AM"),
    "SCMM X":               ("twitter",   "SeniorCareMax",             "09:00 AM"),
    "Welton LinkedIn":      ("linkedin",  "Welton Hong",               "12:00 PM"),
    "Welton Instagram":     ("instagram", "Welton Hong",               "12:00 PM"),
    "Welton X":             ("twitter",   "weltonhong",                "12:00 PM"),
}
...
def phase2_contentstudio_qa(posts, phase1):
    ...
    for post in posts:
        ...
        # Hashtags
        if is_x:
            has_tags = any(h in post_hashtags for h in HOMECARE_HASHTAGS.split())
            log(has_tags, "Hashtags present (trimming allowed on X)", post_hashtags or "NONE FOUND")
        else:
            tags_ok = post_hashtags == HOMECARE_HASHTAGS
            log(tags_ok, "Hashtags correct")
            if not tags_ok:
                print(f"    Expected: {HOMECARE_HASHTAGS}")
...
def main():
    if len(sys.argv) < 2:
        print("Usage:   python qa_workflow_blog_homecare_scmm.py <date>")
        print("Example: python qa_workflow_blog_homecare_scmm.py 2026-05-18")
        sys.exit(1)
    ...
    print("=" * 60)
    print(f"  Blog QA — Senior Care Marketing Max (Home Care)")
    print(f"  Posting date:  {date_str}")
    print(f"  Article date:  {article_date} (3 weeks prior)")
    print("=" * 60)

    print("\nFetching posts from ContentStudio...")
    all_posts = []
    for page in range(1, 10):
        data = cs_get(f"/workspaces/{WS_ID}/posts?status=review&page={page}")
    ...
    homecare_posts = [
        ...
        if TIME_9AM in p["scheduling"]["execute_time"]
        or TIME_12PM in p["scheduling"]["execute_time"]
    ]
    print(f"Found {len(homecare_posts)} Home Care post(s) for {date_str}")

    if not homecare_posts:
        print("\n[WARN] No Home Care posts found at 9 AM or 12 PM PST for this date.")
        sys.exit(0)

    # Get blog URL from SCMM Instagram post (9 AM standalone)
    scmm_ig_post = next(
        (p for p in homecare_posts
         if TIME_9AM in p["scheduling"]["execute_time"]
    ...
    # Fallback: any 9 AM instagram post
    if not scmm_ig_post:
        scmm_ig_post = next(
            (p for p in homecare_posts
             if TIME_9AM in p["scheduling"]["execute_time"]
    ...
    if p1:
        phase2_contentstudio_qa(homecare_posts, p1)
        phase3_schedule_channels(homecare_posts)
