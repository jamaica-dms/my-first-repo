#!/usr/bin/env python3
"""
Blog QA Checker — RRM@home Workspace (Windows & Doors / Window Covering)
Runs 3-phase QA on Windows & Doors and Window Covering blog posts scheduled for a given Monday.

Usage:
    python qa_workflow_blog_windowsdoor_covering_rrmathome.py 2026-05-18
"""
...
# 12 PM PST = 19:00 UTC, 1 PM PST = 20:00 UTC
TIME_12PM = "T19:00:00"
TIME_1PM  = "T20:00:00"

WD_HASHTAGS = "#windowanddoormarketing #windowcoveringmarketing #homeimprovementmarketing #rrmathome"

# (platform, name_fragment, expected_pst_time)
EXPECTED_CHANNELS = {
    "RRM@home Facebook":        ("facebook",  "RRM At Home",  "12:00 PM"),
    "RRM@home LinkedIn":        ("linkedin",  "RRM@home",     "12:00 PM"),
    "Welton RRM@home Facebook": ("facebook",  "Welton Hong",  "12:00 PM"),
    "RRM@home Instagram":       ("instagram", "RRM@home",     "12:00 PM"),
    "RRM@home X":               ("twitter",   "rrmathome",    "12:00 PM"),
    "Welton LinkedIn":          ("linkedin",  "Welton Hong",  "01:00 PM"),
    "Welton Instagram":         ("instagram", "Welton Hong",  "01:00 PM"),
    "Welton X":                 ("twitter",   "weltonhong",   "01:00 PM"),
}
...
def phase2_contentstudio_qa(posts, phase1):
    ...
    for post in posts:
        ...
        if is_x:
            has_tags = any(h in post_hashtags for h in WD_HASHTAGS.split())
            log(has_tags, "Hashtags present (trimming allowed on X)", post_hashtags or "NONE FOUND")
        else:
            tags_ok = post_hashtags == WD_HASHTAGS
            log(tags_ok, "Hashtags correct")
            if not tags_ok:
                print(f"    Expected: {WD_HASHTAGS}")
...
def main():
    if len(sys.argv) < 2:
        print("Usage:   python qa_workflow_blog_windowsdoor_covering_rrmathome.py <date>")
        print("Example: python qa_workflow_blog_windowsdoor_covering_rrmathome.py 2026-05-18")
        sys.exit(1)
    ...
    print("=" * 60)
    print(f"  Blog QA — RRM@home (Windows & Doors / Window Covering)")
    ...
    wd_posts = [
        ...
        if (
            TIME_12PM in p["scheduling"]["execute_time"]
            and not all("welton" in a["name"].lower() for a in p["accounts"])
        )
        or (
            TIME_1PM in p["scheduling"]["execute_time"]
            and all("welton" in a["name"].lower() for a in p["accounts"])
        )
    ]
    print(f"Found {len(wd_posts)} Windows & Doors post(s) for {date_str}")

    if not wd_posts:
        print("\n[WARN] No Windows & Doors posts found at 12 PM or 1 PM PST for this date.")
        sys.exit(0)

    # Get blog URL from RRM@home Instagram post (12 PM standalone)
    rrmathome_ig_post = next(
        (p for p in wd_posts
         if TIME_12PM in p["scheduling"]["execute_time"]
    ...
    if p1:
        phase2_contentstudio_qa(wd_posts, p1)
        phase3_schedule_channels(wd_posts)
