#!/usr/bin/env python3
"""
Blog QA Checker — Ring Ring Marketing Workspace
Runs 3-phase QA on blog posts scheduled for a given date.

Usage:
    python blog_qa.py 2026-05-18
"""
...
# 7 AM PST = 14:00 UTC, 8 AM PST = 15:00 UTC
CEMETERY_TIME = "T14:00:00"
FUNERAL_TIME  = "T15:00:00"  # not yet in scope

CEMETERY_HASHTAGS = "#cemeterymarketing #cemeterydigitalmarketing #cemeteryoutreach #ringringmarketing"
...
def compare_images(url1, url2):
    ...
    try:
        ...
        note = "install Pillow for visual comparison"
        return ratio >= 0.5, f"Size ratio {ratio:.0%} ({note})"
...
def phase1_website_qa(blog_url):
    ...
    log(not missing_punct, "Punctuation", f"{len(missing_punct)} paragraph(s) missing end punctuation" if missing_punct else "")
...
def phase2_contentstudio_qa(posts, phase1, blog_type):
    print(f"\n-- PHASE 2: CONTENTSTUDIO QA ({blog_type.upper()}) " + "-" * 20)

    expected_hashtags = CEMETERY_HASHTAGS if blog_type == "cemetery" else FUNERAL_HASHTAGS
    ...
    for post in posts:
        ...
        # Hashtags
        if is_x:
            has_tags = any(h in post_hashtags for h in expected_hashtags.split())
            log(has_tags, "Hashtags present (trimming allowed on X)", post_hashtags or "NONE FOUND")
        else:
            tags_ok = post_hashtags == expected_hashtags
...
def phase3_schedule_channels(posts, blog_type):
    print(f"\n-- PHASE 3: SCHEDULE & CHANNEL VERIFICATION ({blog_type.upper()}) " + "-" * 5)

    expected_pst = "07:00 AM" if blog_type == "cemetery" else "08:00 AM"
...
def main():
    if len(sys.argv) < 2:
        print("Usage:   python blog_qa.py <date>")
        print("Example: python blog_qa.py 2026-05-18")
        sys.exit(1)
    ...
    print("=" * 60)
    print(f"  Blog QA — Ring Ring Marketing")
    print(f"  Date: {date_str}")
    print("=" * 60)

    # Fetch all in-review posts
    ...
    print(f"Found {len(date_posts)} in-review post(s) for {date_str}")

    # -- Cemetery (7 AM PST) --------------------------------------
    cem_posts = [p for p in date_posts if CEMETERY_TIME in p["scheduling"]["execute_time"]]

    if not cem_posts:
        print("\n[WARN]  No Cemetery posts found at 7 AM for this date.")
    else:
        print(f"\n{'='*60}")
        print("  CEMETERY BLOG")
        print(f"{'='*60}")
        ...
        ig_post = next(
            (p for p in cem_posts if any(a["platform"] == "instagram" for a in p["accounts"])),
            None
        )
        ...
        if not ig_post:
            ...
            if not blog_url:
                ...
                if p1:
                    phase2_contentstudio_qa(cem_posts, p1, "cemetery")
                    phase3_schedule_channels(cem_posts, "cemetery")
