#!/usr/bin/env python3
"""
Blog QA Checker — RRM@home Workspace (Flooring)
Runs 3-phase QA on Flooring blog posts scheduled for a given Monday.

Usage:
    python qa_workflow_blog_flooring_rrmathome.py 2026-05-18
"""
...
BLOG_DELAY = timedelta(days=14)   # article published 2 weeks before posting date

# 9 AM PST = 16:00 UTC, 10 AM PST = 17:00 UTC
TIME_9AM  = "T16:00:00"
TIME_10AM = "T17:00:00"

FLOORING_HASHTAGS = "#flooringmarketing #flooringbusinessgrowth #flooringdigitalmarketing #rrmathome"

# (platform, name_fragment, expected_pst_time)
EXPECTED_CHANNELS = {
    "RRM@home Facebook":        ("facebook",  "RRM At Home",  "09:00 AM"),
    "RRM@home LinkedIn":        ("linkedin",  "RRM@home",     "09:00 AM"),
    "Welton RRM@home Facebook": ("facebook",  "Welton Hong",  "09:00 AM"),
    "RRM@home Instagram":       ("instagram", "RRM@home",     "09:00 AM"),
    "RRM@home X":               ("twitter",   "rrmathome",    "09:00 AM"),
    "Welton LinkedIn":          ("linkedin",  "Welton Hong",  "10:00 AM"),
    "Welton Instagram":         ("instagram", "Welton Hong",  "10:00 AM"),
    "Welton X":                 ("twitter",   "weltonhong",   "10:00 AM"),
}
...
def phase2_contentstudio_qa(posts, phase1):
    ...
    for post in posts:
        ...
        # Hashtags
        if is_x:
            has_tags = any(h in post_hashtags for h in FLOORING_HASHTAGS.split())
            log(has_tags, "Hashtags present (trimming allowed on X)", post_hashtags or "NONE FOUND")
        else:
            tags_ok = post_hashtags == FLOORING_HASHTAGS
            log(tags_ok, "Hashtags correct")
            if not tags_ok:
                print(f"    Expected: {FLOORING_HASHTAGS}")
...
def main():
    if len(sys.argv) < 2:
        print("Usage:   python qa_workflow_blog_flooring_rrmathome.py <date>")
        print("Example: python qa_workflow_blog_flooring_rrmathome.py 2026-05-18")
        sys.exit(1)
    ...
    print("=" * 60)
    print(f"  Blog QA — RRM@home (Flooring)")
    print(f"  Posting date:  {date_str}")
    print(f"  Article date:  {article_date} (2 weeks prior)")
    print("=" * 60)
    ...
    print(f"Found {len(flooring_posts)} Flooring post(s) for {date_str}")

    if not flooring_posts:
        print("\n[WARN] No Flooring posts found at 9 AM or 10 AM PST for this date.")
        sys.exit(0)

    # Get blog URL from RRM@home Instagram post (9 AM standalone)
    rrmathome_ig_post = next(
        (p for p in flooring_posts
         if TIME_9AM in p["scheduling"]["execute_time"]
         and len(p["accounts"]) == 1
         and p["accounts"][0]["platform"] == "instagram"
         and "rrm" in p["accounts"][0].get("name", "").lower()),
        None
    )
    ...
    if p1:
        phase2_contentstudio_qa(flooring_posts, p1)
        phase3_schedule_channels(flooring_posts)
